<template>
    <!-- 开始使用 -->
     <div class="flex flex-col items-center justify-start mt-[80px] min-h-[334px] w-full pb-[120px]">
            <div class="bg-gradient-to-b from-[#f2fff1] from-[3%] to-white border border-[#f3f3f3] rounded-lg box-border flex justify-between min-h-[340px] px-[60px] pb-[20px] w-[1200px]">
                <div class="felx-1">
                    <h1 data-aos="fade-up" class="text-primary-light text-[36px] font-semibold leading-[56px] mt-[54px] text-left w-[520px]">ChatX服务于全国的跨国创业者们，助您精准表达，无缝沟通。</h1>
                    <p data-aos="fade-up" class="text-[16px] text-[#676c76] text-base font-medium leading-6 text-left mt-[16px]">无论您对于ChatX的产品定价、方案定制或其他任何方面的问题都可以随时联系我们</p>
                </div>
                <div class="flex flex-col items-end mr-[20px]" data-aos="fade-up">
                    <img src="../public/ipx/cx-index-end-pic.webp" class="w-[242px] h-[62px] mt-[64px]"/>
                    <div class="flex justify-between mt-[78px]">
                        <img src="/ipx/index-btn-icon-2.webp" class="h-[48px] w-[48px] mr-[24px]"/>
                        <button class="text-[20px] min-h-[64px] w-[240px] bg-[#22242c] rounded-[4px] text-[#fff] cursor-pointer leading-[20px] px-5 py-2 transition-all duration-300">开始使用</button>
                    </div>
                </div>
             </div>
      </div>
</template>